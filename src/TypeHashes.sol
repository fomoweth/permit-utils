...
library TypeHashes {
	...
	bytes32 internal constant PERMIT_BATCH_TYPEHASH =
		...
		return
			keccak256(
				abi.encode(
					...
					_hashPermitDetails(permit.details),
					permit.spender,
					permit.sigDeadline
				)
			);
	...
	function _hashPermitDetails(IAllowanceTransfer.PermitDetails[] memory details) private pure returns (bytes32) {
		bytes memory hashes;
		for (uint256 i; i < details.length; ++i) hashes = bytes.concat(hashes, _hashPermitDetails(details[i]));
	...
	function _hashTokenPermissions(
		...
		bytes memory hashes;
		for (uint256 i; i < permitted.length; ++i) hashes = bytes.concat(hashes, _hashTokenPermissions(permitted[i]));
		return keccak256(hashes);
	}
}
